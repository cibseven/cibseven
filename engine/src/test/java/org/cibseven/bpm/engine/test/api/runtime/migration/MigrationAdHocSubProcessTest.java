...
public class MigrationAdHocSubProcessTest {
  ...
  /** A gate before the scope, so an instance can stand outside it while the scope exists. */
  protected static BpmnModelInstance adHocProcessBehindAGate() {
    AdHocSubProcessBuilder adHoc = Bpmn.createExecutableProcess("Process")
        .startEvent("start")
        .userTask("gate")
        .adHocSubProcess("adHoc");
  ...
  public void migratingAnActiveAdHocScopeIsRefusedAtExecutionTime() {
    ...
    try {
      ...
      assertThat(e.getMessage())
          .as("the report must name the element at fault, not only its type (CIB7-1857)")
          .contains("Ad hoc sub process 'adHoc' cannot be migrated while it is active")
          .contains("has no defined migration");
      // The generic refusal from SupportedActivityInstanceValidator is still there and still the
      // thing that actually refuses; the sentence above is what tells the reader where to look.
      assertThat(e.getMessage())
  ...
  /**
   * The counterpart to the refusal above, and the reason it was not moved to plan validation
   * (CIB7-1857): an instance that is not standing inside the scope migrates perfectly well under a
   * plan that maps it. Refusing the plan would refuse this too, and tightening a refusal is a
   * breaking change where relaxing one is additive.
   */
  @Test
  public void anInstanceOutsideTheAdHocScopeMigratesUnderAPlanThatMapsIt() {
    ProcessDefinition source = testHelper.deployAndGetDefinition(adHocProcessBehindAGate());
    ProcessDefinition target = testHelper.deployAndGetDefinition(adHocProcessBehindAGate());

    // The instance waits at the gate, so the ad hoc scope has not been entered: starting the
    // process is not enough to be inside it, which is what makes this case distinct from the one
    // above.
    ProcessInstance pi = rule.getRuntimeService().startProcessInstanceById(source.getId());

    MigrationPlan plan = rule.getRuntimeService()
        .createMigrationPlan(source.getId(), target.getId())
        .mapActivities("gate", "gate")
        .mapActivities("adHoc", "adHoc")
        .build();
    ...
    assertThat(rule.getRuntimeService().createProcessInstanceQuery()
        .processInstanceId(pi.getId()).singleResult().getProcessDefinitionId())
        .as("an instance outside the scope must migrate")
