...
public abstract class PvmAtomicOperationActivityInstanceStart extends AbstractPvmEventAtomicOperation {
  ...
  protected PvmExecutionImpl eventNotificationsStarted(PvmExecutionImpl execution) {
    ...
    // activity-start path only, so it holds the activity that was started rather than whatever that
    // activity goes on to do. The first write wins: entering a scope hands the same sink to a second
    // execution, and the PVM runs the activity after the scope on the one that armed it.
    AtomicReference<String> sink = execution.getEnteredActivityInstanceIdSink();
    if (sink != null) {
      sink.compareAndSet(null, execution.getActivityInstanceId());
