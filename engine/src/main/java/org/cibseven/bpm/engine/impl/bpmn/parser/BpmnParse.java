...
import static org.cibseven.bpm.engine.impl.bpmn.parser.BpmnParseUtil.findCamundaExtensionElement;
import static org.cibseven.bpm.engine.impl.bpmn.parser.BpmnParseUtil.parseCamundaExtensionProperties;
import static org.cibseven.bpm.engine.impl.bpmn.parser.BpmnParseUtil.parseCamundaScript;
import static org.cibseven.bpm.engine.impl.bpmn.parser.BpmnParseUtil.parseInputOutput;
import static org.cibseven.bpm.engine.impl.util.ClassDelegateUtil.instantiateDelegate;
...
import org.cibseven.bpm.engine.delegate.VariableListener;
import org.cibseven.bpm.engine.impl.Condition;
import org.cibseven.bpm.engine.impl.HistoryTimeToLiveParser;
import org.cibseven.bpm.engine.impl.ProcessEngineLogger;
import org.cibseven.bpm.engine.impl.bpmn.behavior.BoundaryConditionalEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.BoundaryEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.CallActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.CallableElementActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.CancelBoundaryEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.CancelEndEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.CaseCallActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ClassDelegateActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.CompensationEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.DmnBusinessRuleTaskActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ErrorEndEventActivityBehavior;
...
import org.cibseven.bpm.engine.impl.bpmn.behavior.EventSubProcessStartConditionalEventActivityBehavior;
...
import org.cibseven.bpm.engine.impl.bpmn.behavior.InclusiveGatewayActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.IntermediateCatchEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.IntermediateCatchLinkEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.IntermediateConditionalEventBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.IntermediateThrowNoneEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.MailActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ManualTaskActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.MultiInstanceActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.NoneEndEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.NoneStartEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ParallelGatewayActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ParallelMultiInstanceActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ReceiveTaskActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ScriptTaskActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.SequentialMultiInstanceActivityBehavior;
...
import org.cibseven.bpm.engine.impl.bpmn.behavior.ThrowEscalationEventActivityBehavior;
import org.cibseven.bpm.engine.impl.bpmn.behavior.ThrowSignalEventActivityBehavior;
...
import org.cibseven.bpm.engine.impl.bpmn.listener.ClassDelegateExecutionListener;
import org.cibseven.bpm.engine.impl.bpmn.listener.DelegateExpressionExecutionListener;
import org.cibseven.bpm.engine.impl.bpmn.listener.ExpressionExecutionListener;
import org.cibseven.bpm.engine.impl.bpmn.listener.ScriptExecutionListener;
...
import org.cibseven.bpm.engine.impl.core.variable.mapping.value.NullValueProvider;
...
import org.cibseven.bpm.engine.impl.el.UelExpressionCondition;
...
import org.cibseven.bpm.engine.impl.form.handler.DefaultTaskFormHandler;
import org.cibseven.bpm.engine.impl.form.handler.DelegateStartFormHandler;
import org.cibseven.bpm.engine.impl.form.handler.DelegateTaskFormHandler;
...
import org.cibseven.bpm.engine.impl.jobexecutor.AsyncAfterMessageJobDeclaration;
import org.cibseven.bpm.engine.impl.jobexecutor.AsyncBeforeMessageJobDeclaration;
import org.cibseven.bpm.engine.impl.jobexecutor.EventSubscriptionJobDeclaration;
import org.cibseven.bpm.engine.impl.jobexecutor.JobDeclaration;
import org.cibseven.bpm.engine.impl.jobexecutor.MessageJobDeclaration;
...
import org.cibseven.bpm.engine.impl.pvm.process.HasDIBounds;
import org.cibseven.bpm.engine.impl.pvm.process.Lane;
import org.cibseven.bpm.engine.impl.pvm.process.LaneSet;
import org.cibseven.bpm.engine.impl.pvm.process.ParticipantProcess;
...
import org.cibseven.bpm.engine.impl.scripting.ScriptCondition;
import org.cibseven.bpm.engine.impl.scripting.engine.ScriptingEngines;
import org.cibseven.bpm.engine.impl.task.TaskDecorator;
import org.cibseven.bpm.engine.impl.task.TaskDefinition;
import org.cibseven.bpm.engine.impl.task.listener.ClassDelegateTaskListener;
import org.cibseven.bpm.engine.impl.task.listener.DelegateExpressionTaskListener;
import org.cibseven.bpm.engine.impl.task.listener.ExpressionTaskListener;
import org.cibseven.bpm.engine.impl.task.listener.ScriptTaskListener;
import org.cibseven.bpm.engine.impl.util.DecisionEvaluationUtil;
import org.cibseven.bpm.engine.impl.util.ReflectUtil;
import org.cibseven.bpm.engine.impl.util.ScriptUtil;
...
public class BpmnParse extends Parse {
  ...
  public static final String AD_HOC_ENTRY_ACTIVITIES_PROPERTY = "activeActivityIds";

  /**
   * Accepted as an alias for {@link #AD_HOC_ENTRY_ACTIVITIES_PROPERTY}, because another
   * implementation of this element spells the same extension property this way and a model written
   * against that spelling should run here unchanged. Our own name is preferred: the specification calls the children Activities,
  ...
  public ActivityImpl parseAdHocSubProcess(Element adHocElement, ScopeImpl scope) {
    ...
    if (ordering != null && "Sequential".equals(ordering.trim())) {
      addError("Ad hoc sub process '" + activity.getId()
          + "': ordering='Sequential' is not yet supported; only 'Parallel' is implemented",
          adHocElement);
    }
    ...
    // Reject inner sequence flows: re-evaluating the completion condition on a child *transition*
    // would require a PVM change, since no transition atomic operation notifies the composite scope.
    // BPMN 2.0.0 section 10.3.5 permits them, so this is a known deviation, and the message says so
    // rather than reading as though the model were malformed.
    for (Element flow : adHocElement.elements("sequenceFlow")) {
      addError("Ad hoc sub process '" + activity.getId() + "': sequence flow '" + flow.attribute("id")
          + "' connects two flow nodes inside an ad hoc sub process, which this engine does not yet"
          + " support. Remove the flow; the activities can be performed in any order without it.",
          flow);
    }

    // A gateway routes along outgoing sequence flows, and the loop above refuses those, so a gateway
    // inside an ad hoc scope can never route anywhere. Left to the generic flow-node validation it
    // surfaced as "Exclusive Gateway 'gw' has no outgoing sequence flows", which tells a modeller
    // to add a flow that this engine would then reject. Name the real constraint instead.
    for (String gatewayTag : GATEWAY_ELEMENT_NAMES) {
      for (Element gateway : adHocElement.elements(gatewayTag)) {
        addError("Ad hoc sub process '" + activity.getId() + "': " + gatewayTag + " '"
            + gateway.attribute("id") + "' cannot be used inside an ad hoc sub process, because a"
            + " gateway routes along sequence flows and sequence flows between child activities are"
            + " not yet supported here.", gateway);
      }
    }
   ...
   */
  /**
   * Declarative entry activation (CIB7-1891): {@code camunda:property activeActivityIds} names the
  ...
  protected void parseAdHocEntryActivation(Element adHocElement, ActivityImpl activity,
    ...
    String usedName = AD_HOC_ENTRY_ACTIVITIES_PROPERTY;
    if (raw == null || raw.trim().isEmpty()) {
      raw = extensionProperties.get(AD_HOC_ENTRY_ACTIVITIES_ALIAS);
      usedName = AD_HOC_ENTRY_ACTIVITIES_ALIAS;
    }
    if (raw == null || raw.trim().isEmpty()) {
      return;
    }
    ...
    if (!raw.contains("${") && !raw.contains("#{")) {
      ...
      if (!unknown.isEmpty()) {
        addError("Ad hoc sub process '" + activity.getId() + "': " + usedName
