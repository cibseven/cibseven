...
import static org.cibseven.bpm.engine.impl.bpmn.helper.CompensationUtil.SIGNAL_COMPENSATION_DONE;
import static org.cibseven.bpm.engine.impl.pvm.runtime.ActivityInstanceState.ENDING;
...
import org.cibseven.bpm.engine.impl.form.FormPropertyHelper;
...
import org.cibseven.bpm.engine.impl.persistence.entity.DelayedVariableEvent;
import org.cibseven.bpm.engine.impl.persistence.entity.IncidentEntity;
...
import org.cibseven.bpm.engine.impl.pvm.delegate.SignallableActivityBehavior;
...
import org.cibseven.bpm.engine.impl.tree.ExecutionWalker;
import org.cibseven.bpm.engine.impl.tree.FlowScopeWalker;
import org.cibseven.bpm.engine.impl.tree.LeafActivityInstanceExecutionCollector;
import org.cibseven.bpm.engine.impl.tree.ReferenceWalker;
import org.cibseven.bpm.engine.impl.tree.ScopeCollector;
import org.cibseven.bpm.engine.impl.tree.ScopeExecutionCollector;
...
public abstract class PvmExecutionImpl extends CoreExecution implements
   ...
   * there.
   *
   * <p>The slot is shared rather than owned, which is why it is written once and never overwritten.
   * A scope activity is entered by an execution created for it, so the slot is handed down to that
   * execution; the execution that was armed keeps its reference, and the PVM runs whatever follows
   * the scope on exactly that execution once the scope is destroyed. Clearing the field on the
   * execution that filled the slot therefore does not disarm the others, and only the first write
   * makes the answer the activity that was asked for rather than the one after it.
   *
   * <p>Transient and null except while a command is waiting for one such id, so nothing here is
   * state.
   */
