...
set "JBOSS_HOME=%CD%\server\wildfly-${version.wildfly28}"
...
if not exist "%FILE%" (
    ...
    > "%FILE%" echo.cibseven.webclient.authentication.jwtSecret=!RANDOM_STRING!
...
echo "starting CIB seven ${project.version} on Wildfly Application Server ${version.wildfly28}"

cd server\wildfly-${version.wildfly28}\bin\
start standalone.bat

ping -n 5 localhost > NULL
start http://localhost:8080/cibseven-welcome/index.html
